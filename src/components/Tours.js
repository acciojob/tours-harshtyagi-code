...
const Tours = () => {
    ...
    const [texts, setTexts] = useState(toursData.map((data) => setInfo(data.info)));
    ...
    return (
        <div>
            { isBtnClick ? "" : <button onClick={() => setIsBtnClick(true)}> Show All Tours </button> }
            <div className="tours">
                {
                    isBtnClick && (
                        dataArr.length !== 0 ? dataArr.map((data, index) => (
                            <div className="single-tour">
                                ...
                                { buttons[index] === 0 ? <button onClick={() => handle(data.info, index, 200)}> Show More </button> : <button onClick={() => handle(data.info, index, 0)}> See Less </button> }
                                <button className="delete-btn" cl onClick={() => handleDelete(index)}> Delete </button>
                            </div>
